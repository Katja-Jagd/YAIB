...
# -*- coding: utf-8 -*-
"""
Unified fine-tuning dispatcher script.

This script automatically routes to the appropriate task-specific fine-tuning script
based on the --task argument:

Task Mapping:
- Mortality24, Mortality -> fine_tuning_classification.py (patient-level)
- Sepsis, AKI -> fine_tuning_classification_per_timestep.py (timestep-level)
- KidneyFunction -> fine_tuning_regression.py (patient-level)
- LengthOfStay -> fine_tuning_regression_per_timestep.py (timestep-level)

Usage:
    python icu_benchmarks/fine_tuning.py --task Mortality24 --model_path ... [other args]
...
TASK_SCRIPT_MAPPING = {
    ...
    "Mortality24": "fine_tuning_classification.py",
    "Mortality": "fine_tuning_classification.py",

    # Timestep-level classification tasks
    "Sepsis": "fine_tuning_classification_per_timestep.py",
    "AKI": "fine_tuning_classification_per_timestep.py",

    # Patient-level regression tasks
    "KidneyFunction": "fine_tuning_regression.py",

    # Timestep-level regression tasks
    "LengthOfStay": "fine_tuning_regression_per_timestep.py",
}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Unified fine-tuning dispatcher - routes to task-specific scripts",
